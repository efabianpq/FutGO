...
<div class="max-w-3xl mx-auto px-4 py-6" x-data="{ shareUrl: window.location.href, shareTitle: @js($tournament->name) }">
    ...
    <div class="bg-white border border-line rounded-md shadow-card-2 p-5 mb-5">
        ...
        <div class="flex flex-wrap items-center gap-2 mt-4 pt-4 border-t border-line-soft">
            {{-- FutGO Social: seguir el torneo (solo usuarios autenticados) --}}
            <x-social.follow-button :followable="$tournament" type="tournament" />
            {{-- Botón WhatsApp: copia el link directamente para compartir --}}
            <a href="https://wa.me/?text={{ urlencode($tournament->name . ' — Sigue el torneo en FutGO: ' . url()->current()) }}"
               target="_blank" rel="noopener"
               class="inline-flex items-center gap-1.5 bg-[#25D366] text-white font-display font-bold text-[12px] uppercase tracking-wide-label px-3 py-2 rounded-md hover:opacity-90">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 shrink-0" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 0C5.373 0 0 5.373 0 12c0 2.121.552 4.112 1.518 5.845L0 24l6.335-1.518A11.952 11.952 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 22c-1.896 0-3.67-.502-5.204-1.381l-.374-.222-3.763.902.919-3.667-.243-.386A9.96 9.96 0 012 12C2 6.477 6.477 2 12 2s10 4.477 10 10-4.477 10-10 10z"/></svg>
                WhatsApp
            </a>
            <button type="button"
                    ...
                    @foreach (['goleadores' => 'Goleadores', 'posiciones' => 'Posiciones'] as $tipo => $label)
                        <div class="flex items-center justify-between px-3 py-1.5 hover:bg-bone-soft">
                            ...
                            <span class="flex gap-2">
                                <a href="{{ route('torneos.public.img', [$tournament, $tipo]) }}" target="_blank" class="font-mono text-[11px] text-pitch-mist font-bold">SVG</a>
                                <a href="{{ route('torneos.public.img.png', [$tournament, $tipo]) }}" download class="font-mono text-[11px] text-gol-deep font-bold">PNG</a>
                            </span>
                        </div>
                    @endforeach
                    @if ($tournament->mvpEnabled())
                        <div class="flex items-center justify-between px-3 py-1.5 hover:bg-bone-soft">
                            <span class="text-[13px] text-ink">MVP de la fecha</span>
                            <span class="flex gap-2">
                                <a href="{{ route('torneos.public.img', [$tournament, 'mvp']) }}" target="_blank" class="font-mono text-[11px] text-pitch-mist font-bold">SVG</a>
                                <a href="{{ route('torneos.public.img.png', [$tournament, 'mvp']) }}" download class="font-mono text-[11px] text-gol-deep font-bold">PNG</a>
                            </span>
                        </div>
                    @endif
                </div>
                    ...
                    @auth
                        ...
                        @else
                            <a href="{{ route('torneos.mis-equipos') }}"
                               class="inline-flex items-center gap-1.5 bg-gol-deep text-bone font-display font-bold text-[13px] uppercase tracking-wide-label px-5 py-2.5 rounded-md hover:opacity-90">
                                Crea tu equipo para inscribirte
                            </a>
                        @endif
                    @else
                        <a href="{{ route('login') }}"
