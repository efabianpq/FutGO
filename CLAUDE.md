...
**Feed** (`FeedService` + `Social\FeedController`, `/feed`): relevancia calculada en lectura (sin filas por usuario), estrictamente personal — **nunca se distribuye por ciudad** (eso ya lo cubre la exploración con filtro de ciudad en `/oportunidades`; difundir por ciudad en el Feed lo llenaría de eventos sin relación real con el usuario). Un evento es relevante si: (1) el usuario ES el actor/subject — directamente como `user`, o como capitán de un club que lo sea (`ownedKeys()`, garantiza que una notificación dirigida como "respondieron tu oportunidad" siempre llegue al dueño aunque nadie lo siga); o (2) el usuario sigue al actor o al subject (`followedKeys()`). Feed vacío (sin follows) → oportunidades de entrada de su ciudad, como bloque de exploración explícito, no como notificación. Paginado (15/pág).

Generación de eventos no bloqueante (`FeedService::record()` atrapa toda excepción): oportunidad_publicada/respondida/aceptada, amistoso_confirmado, resultado_amistoso, resultado_torneo, logro_desbloqueado. `oportunidad_respondida`: actor=respondente (club o user), subject=dueño de la oportunidad (club o user) — así el dueño ve "te respondieron" y un seguidor del dueño/respondente ve el contexto completo (quién, a quién, sobre qué tipo de oportunidad).
...
| 6 | Convocatoria previa no pre-llena la alineación | ✅ Resuelta (TX-4) — `MatchResultController::show` consulta `match_call_ups` confirmados; la planilla pre-marca solo esos jugadores. Sin convocatoria previa cargada, **nadie sale premarcado** ("jugó"/"titular" arrancan sin check): quien diligencia la planilla decide (cambio 2026-07-03, antes el fallback marcaba todo el plantel) |
